...
// Mock ethers module first to avoid hoisting issues
jest.mock('ethers', () => ({
...
describe('Wallet Core Implementation', () => {
  ...
  beforeEach(() => {
    ...
    // Mock Contract constructor to return mock contract with proper methods
  ...
  describe('Transaction Type Compatibility', () => {
    ...
    it('should handle TransactionRequest conversion with all types', async () => {
      ...
      expect(mockSigner.sendTransaction).toHaveBeenCalledWith({
        ...
        data: '0x123456',
        gasLimit: BigInt(21000)
        // Note: gasPrice is not included in the converted transaction format for Simple Transaction
      });
    });
  ...
  describe('Balance Operations', () => {
    ...
    it('should get token balance using bracket notation', async () => {
      // Use a different contract address to test regular ERC20 token
      const balance = await wallet.getTokenBalance('0x1234567890123456789012345678901234567890');
  ...
  describe('Advanced OmniCoin Features', () => {
    ...
    it('should unstake OmniCoin with encoded function call', async () => {
      ...
      expect(result.hash).toBe('0x' + '1'.repeat(64));
      expect(mockEthers.id).toHaveBeenCalledWith('unstake(uint256)');
      expect(mockEthers.AbiCoder.defaultAbiCoder().encode).toHaveBeenCalled();
    });
  ...
  describe('Privacy Features', () => {
    ...
    it('should create privacy account', async () => {
      ...
      expect(result.hash).toBe('0x' + '1'.repeat(64));
      expect(mockEthers.randomBytes).toHaveBeenCalledWith(32);
    ...
    it('should get privacy balance', async () => {
      ...
      expect(balance).toBe(BigInt('123'));
      expect(mockEthers.getBytes).toHaveBeenCalled();
      expect(mockEthers.keccak256).toHaveBeenCalled();
    });
  ...
  describe('Governance Features', () => {
    ...
    it('should propose governance action', async () => {
      ...
      expect(result.hash).toBe('0x' + '1'.repeat(64));
      expect(mockEthers.id).toHaveBeenCalledWith('propose(address[],uint256[],bytes[],string)');
      expect(mockEthers.toUtf8Bytes).toHaveBeenCalledWith('Test proposal');
    });
    ...
    it('should vote on proposal', async () => {
      ...
      expect(result.hash).toBe('0x' + '1'.repeat(64));
      expect(mockEthers.id).toHaveBeenCalledWith('castVote(uint256,uint8)');
    });
    ...
    it('should vote against proposal', async () => {
      ...
      expect(mockEthers.AbiCoder.defaultAbiCoder().encode).toHaveBeenCalledWith(
        ['uint256', 'uint8'],
        [1, 0]
      );
    });
  ...
  describe('Event Handling', () => {
    ...
    it('should register account change callback', async () => {
      ...
      await handleAccountChange.call(wallet, ['0xnewaddress']);
    ...
    it('should register network change callback', async () => {
      ...
      await handleNetworkChange.call(wallet, '0x89'); // Polygon chainId
  ...
  describe('BigInt and Native Integration', () => {
    ...
    it('should handle ethers v6 parseEther compatibility', () => {
      const parsed = mockEthers.parseEther('1.5');
      expect(parsed).toBe(BigInt('1500000000000000000'));
    });

    it('should handle ethers v6 formatEther compatibility', () => {
      const formatted = mockEthers.formatEther(BigInt('1500000000000000000'));
