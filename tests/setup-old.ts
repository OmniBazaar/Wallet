...
const createMockIDBDatabase = () => {
  ...
  return {
    objectStoreNames: { 
      contains: jest.fn((name: string) => stores.has(name))
    },
    ...
    transaction: jest.fn((storeNames: string | string[], mode: 'readonly' | 'readwrite' = 'readonly') => {
      ...
      return {
        objectStore: jest.fn((storeName: string) => {
          const store = stores.get(storeName) || { data: new Map() };
...
jest.mock('bitcoinjs-lib', () => ({
  networks: { bitcoin: {} },
  ECPair: { fromWIF: jest.fn() },
  payments: { p2pkh: jest.fn() }
}));
...
jest.mock('elliptic', () => ({
  ec: jest.fn(() => ({
    keyFromPrivate: jest.fn(() => ({
      ...
      sign: jest.fn(() => ({ r: '1', s: '1', recoveryParam: 0 }))
    }))
  }))
}));

// Mock ethers HDNodeWallet to fix hardware wallet test
jest.mock('ethers', () => {
  ...
  return {
    ...
    HDNodeWallet: {
      ...actual.HDNodeWallet,
      fromPhrase: jest.fn().mockImplementation((mnemonic) => ({
        derivePath: jest.fn().mockImplementation((path) => ({
