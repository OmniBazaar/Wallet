...
describe("ENS Integration Tests", function () {
  ...
  describe("OmniNameRegistry", function () {
    ...
    it("Should prevent registration of reserved names", async function () {
      ...
      await expect(
        nameRegistry.connect(user1).register("admin", { value: fee })
  ...
  describe("Oracle Integration", function () {
    ...
    it("Should track oracle health", async function () {
      // Update name to set initial last update time
      await oracle.connect(node1).updateName("alice", user1.address);
  ...
  describe("Stateless Resolver", function () {
    ...
    it("Should handle cache updates", async function () {
      // The oracle is already authorized in beforeEach
      // Let's use node1 as the signer but authorize it as an oracle
      await resolver.connect(owner).addOracle(node1.address);
      
      // Update cache using authorized oracle signer
      await resolver.connect(node1).updateCache("charlie", owner.address);
  ...
  describe("End-to-End Integration", function () {
    it("Should simulate full name resolution flow", async function () {
      ...
      // 5. Wait for cooldown period (simulate time passage)
      await ethers.provider.send("evm_increaseTime", [61]); // 61 seconds (cooldown is 60)
      await ethers.provider.send("evm_mine");
      
      // 6. Node updates oracle (after cooldown)
      await oracle.connect(node1).updateName("alice", user2.address);
      
      // 7. Resolver returns updated address
