...
const REQUIRES_APPROVAL_RESET = {
  'ethereum': [] // OmniCoin doesn't need approval reset
}
        ...
        <div className="PaddingBottomS StepsWrapper">
          ...
          {showSyncWaiting && (
            <a
              href={
                transaction
                  ? link({
                      url: `https://scan.omnibazaar.com/tx/${transaction.blockchain}/${transaction.id}?sender=${payment.route.fromAddress}&receiver=${payment.route.toAddress}&deadline=${transaction.deadline}`,
                      target: '_blank',
                      wallet,
                    })
                  : undefined
          ...
          )}
          {
            ...
            <div className="Step Card small transparent disabled active">
              ...
              <span>Need help?&nbsp;</span>
              <a
                href={link({ url: `https://support.omnibazaar.com?wallet=${encodeURIComponent(wallet?.name)}&account=${account}&transaction=${transaction?.id}&query=${encodeURIComponent(`Problem with payment`)}`, target: '_blank', wallet })}
                target="_blank"
                className="Link"
              >Contact support</a>
              </div>
