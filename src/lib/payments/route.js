...
import OmniCoinPaymentRouter from './omnicoin-router'
...
async function remoteRouteToPaymentRoute({ remoteRoute, from, accept }) {
  ...
  // Check if this is an OmniCoin transaction
  const isOmniCoin = fromToken.address.toLowerCase() === config.omnicoinAddress.toLowerCase() || 
                    toToken.address.toLowerCase() === config.omnicoinAddress.toLowerCase()
  ...
  // Use OmniCoinPaymentRouter for OmniCoin transactions
  const PaymentRouterClass = isOmniCoin ? OmniCoinPaymentRouter : PaymentRoute

  let paymentRoute = new PaymentRouterClass({
    blockchain: remoteRoute['blockchain'],
    fromAddress,
    fromToken,
    fromBalance,
  ...
  // For OmniCoin transactions, add privacy and staking if enabled
  if (isOmniCoin) {
    if (config.omnicoinConfig.privacyEnabled) {
      await paymentRoute.enablePrivacy()
    }
    if (config.omnicoinConfig.stakingEnabled) {
      await paymentRoute.enableStaking()
    }
  }
